...
(function ($, window, document, undefined) {
	...
	Plugin.prototype.togglePanel = function (event) {
		...
		if (plugin.options.autoCollapse) { // expand current panel and collapse the rest
			...
			plugin.headers.each(function (i, el) {
				...
				if ($btn[0] != $(event.currentTarget)[0]) {
					...
					/*
						...
						This section refers to all inactive panels.
						You have correctly added the aria-expanded attribute.
						You also need to change the aria-hidden attribute of the panel ($hdr.next()) to true.
					...
					*/
					...
					/*  
						...
						This section refers to the currently selected accordion header. 
						You need to set the aria-expanded attribute of the button to true.
						You also need to change the aria-hidden attribute of the panel ($hdr.next()) to false.
			...
			// $btn.attr('aria-expanded','false');
			// $panel.next().attr('aria-hidden','false');
			$panel.slideToggle({
			...
			/*
				...
				If the panel is currently visible set the aria-expanded attribute of the button to false,
				otherwise set it to true.
				Don't forget to change the aria-hidden attribute of the panel ($panel) accordingly.
